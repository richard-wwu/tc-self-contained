 ...
 */
__isl_give EL *FN(FN(PW,get),BASE)(__isl_keep PW *pw, int pos)
{
	if (FN(PW,check_pos) < 0)
		return NULL;
 ...
 * The only exception is that isl_pw_*_free can be called instead.
 */
__isl_give EL *FN(FN(PW,take),BASE)(__isl_keep PW *pw, int pos)
{
	EL *el;
	...
	if (!pw)
		return NULL;
	if (pw->ref != 1)
		return FN(FN(PW,get),BASE)(pw, pos);
	if (FN(PW,check_pos) < 0)
		return NULL;
	el = pw->p[pos].FIELD;
	pw->p[pos].FIELD = NULL;
 ...
 * due to a preceding call to isl_pw_*_take_*.
 * However, in this case, "pw" only has a single reference and
 * then the call to isl_pw_*_cow has no effect.
 */
__isl_give PW *FN(FN(PW,restore),BASE)(__isl_take PW *pw, int pos,
	__isl_take EL *el)
{
	if (!pw || !el)
		goto error;
